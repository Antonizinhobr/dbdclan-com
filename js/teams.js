...
document.getElementById("page-main-title").innerText =
  `EQUIPES DO TORNEIO ${currentCamp}`;
document.getElementById("modal-title-camp").innerText =
  `INSCRIÇÃO - TORNEIO ${currentCamp}`;
...
onSnapshot(doc(db, "sistema", "geral"), (docSnap) => {
  ...
  if (deletedCamps.includes(campInt)) {
    document.querySelector("main").innerHTML =
      '<h2 class="camp-deleted-msg"><i class="fas fa-skull"></i> ESTE CAMPEONATO FOI EXCLUÍDO PELA ENTIDADE.</h2>';
    renderCampDropdown();
    return;
  }
...
function renderCampDropdown() {
  ...
  for (let i = 1; i <= currentTotalCamps; i++) {
    if (!deletedCamps.includes(i)) {
      activeCamps.push(i);
      const isLocked = lockedCamps.includes(i)
        ? ' <i class="fas fa-lock icon-lock-small"></i>'
        : "";

      if (menuTeams)
        menuTeams.innerHTML += `<a href="teams.html?camp=${i}">Campeonato ${i}${isLocked}</a>`;
      if (menuBrackets)
...
window.toggleLockCamp = async () => {
  ...
  if (!isCurrentCampLocked) {
    ...
    alert(
      `O Campeonato ${campInt} foi BLOQUEADO! Ninguém mais pode criar ou excluir times.`,
    );
  }
...
window.deleteCamp = async () => {
  ...
  if (
    !confirm(
      `⚠️ ATENÇÃO EXTREMA: Você está prestes a EXCLUIR o Campeonato ${campInt} e TODOS OS SEUS TIMES.\n\nIsso NÃO pode ser desfeito. Tem certeza?`,
    )
  )
    return;
  ...
  try {
    ...
    await setDoc(
      refDoc,
      { deletedCamps: newDeleted, lockedCamps: newLocked },
      { merge: true },
    );
...
onSnapshot(q, (snapshot) => {
  ...
  snapshot.forEach((docSnap) => {
    ...
    if (data.campId == currentCamp) {
      if (data.leaderEmail)
        registeredEmails.add(data.leaderEmail.toLowerCase());
      if (data.members)
        data.members.forEach((m) =>
          registeredEmails.add(m.email.toLowerCase()),
        );
    }
  });
  ...
  if (window.currentViewedTeamId) {
    const updatedTeam = allTeams.find(
      (t) => t.id === window.currentViewedTeamId,
    );
...
function renderTeamsGrid() {
  ...
  if (filteredTeams.length === 0) {
    grid.innerHTML =
      '<p class="empty-msg">Nenhuma equipe cadastrada para este campeonato ainda.</p>';
    return;
  }
...
window.deleteTeam = async (teamId) => {
  if (isCurrentCampLocked && !isAdmin)
    return alert("Ações bloqueadas! O Campeonato foi encerrado.");
  if (
    !confirm("⚠️ AVISO: Deseja realmente excluir esta equipe permanentemente?")
  )
    return;
  try {
...
window.openViewModal = (team) => {
  ...
  const isCaptain =
    ...
    ((team.leaderUid && userToCheck.uid === team.leaderUid) ||
      (userToCheck.email &&
        userToCheck.email.toLowerCase() === team.leaderEmail.toLowerCase()));
  ...
  elencoCompleto.forEach((person) => {
    ...
    const coroa = person.isCap
      ? '<i class="fas fa-crown icon-crown-cap"></i>'
      : "";
    const caveira = isKiller
      ? '<i class="fas fa-skull icon-skull-killer"></i>'
      : "";

    const avatarClass = isKiller
      ? "is-killer"
      : person.isCap
        ? "is-cap"
        : "is-normal";
    const nameClass = isKiller
      ? "is-killer"
      : person.isCap
        ? "is-cap"
        : "is-normal";
...
window.openCreateTeamModal = () => {
  if (isCurrentCampLocked && !isAdmin)
    return alert("As inscrições para este campeonato estão encerradas!");

  const user = auth.currentUser || currentUser;
  if (!user)
    return alert("Sessão inválida. Por favor, aguarde ou recarregue a página.");

  const userEmail = user.email ? user.email.toLowerCase() : "";
  if (userEmail && registeredEmails.has(userEmail)) {
    return alert(
      "Você já está cadastrado em uma equipe neste campeonato! Cada jogador pode participar de apenas um time.",
    );
  }
...
window.nextTeamStep = () => {
  ...
  const capEmail = document
    .getElementById("t-cap-email")
    .value.trim()
    .toLowerCase();
  const fileInput = document.getElementById("t-logo");

  if (!name || !capName || !capEmail)
    return alert("Preencha o Nome da Equipe, Capitão e E-mail!");

  if (fileInput.files.length > 0) {
    const file = fileInput.files[0];
    if (!file.type.startsWith("image/"))
      return alert(
        "Por favor, selecione um arquivo de imagem válido (JPG ou PNG).",
      );
    if (file.size > 2 * 1024 * 1024)
      return alert(
        "A imagem é muito pesada! O tamanho máximo permitido é 2MB.",
      );
  }

  if (registeredEmails.has(capEmail)) {
    return alert(
      `Erro: O capitão (${capEmail}) já está registrado em outra equipe no torneio atual!`,
    );
  }
...
window.addDraftMember = () => {
  if (draftMembers.length >= 4)
    return alert(
      "Limite de Membros atingido! Uma equipe pode ter no máximo 4 membros além do Capitão (Total de 5 jogadores).",
    );
  ...
  if (mEmail === capEmail)
    return alert("O capitão já faz parte do time automaticamente!");
  if (draftMembers.some((m) => m.email === mEmail))
    return alert("Você já adicionou este e-mail na lista abaixo!");
  if (registeredEmails.has(mEmail))
    return alert(
      `Erro: O e-mail ${mEmail} já pertence a outro time neste torneio! Peça para ele fornecer outro e-mail válido.`,
    );
...
window.renderDraftMembers = () => {
  ...
  if (draftMembers.length === 0) {
    container.innerHTML =
      '<p class="empty-draft-text">Nenhum membro adicionado ainda.</p>';
    return;
  }

  container.innerHTML = draftMembers
    .map(
      (m, i) => `
...
window.submitTeam = async () => {
  ...
  if (draftMembers.length < 4) {
    return alert(
      "Erro: Sua equipe está incompleta! Você precisa adicionar exatamente 4 membros (além do Capitão) para finalizar a inscrição.",
    );
  }
  ...
  btnSubmit.innerHTML =
    '<i class="fas fa-spinner fa-spin"></i> SALVANDO NA NÉVOA...';
  ...
  try {
    ...
    const webhookUrl =
      "https://discord.com/api/webhooks/1524454164904808558/EEBzHbxPgavMeP3RDKm_d4IDPlMJy9BAKtzzaWVjFhjyBN8NklZooRHRCXb432Eyeiei";

    const membersListString = draftMembers
      .map((m) => `🔪 **${m.name}**`)
      .join("\n");
    ...
    alert(
      "Ocorreu um erro no upload da imagem ou ao salvar a equipe. Tente novamente.",
    );
  } finally {
