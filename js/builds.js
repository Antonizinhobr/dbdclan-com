...
const descricoesCustomizadas = {
  ...
  jmyersmemorial:
    "J. Myers Memorial. Aumenta a velocidade de Pegar e Enganchar Sobreviventes em 40% durante a Maldade Encarnada.",
  judithsjournal:
...
async function sendToDiscordWebhook(buildData) {
  ...
  function getOfficialName(path) {
    ...
    if (ADDONS_DB[key]) {
      return ADDONS_DB[key].split(".")[0].trim();
    }
  ...
  function getMyGithubUrl(localPath) {
    ...
    let relativePath = "";
    if (index !== -1) {
      relativePath = localPath.substring(index + marker.length);
    } else if (localPath.indexOf("/img/") !== -1) {
      relativePath = localPath.split("/img/")[1];
      if (relativePath.startsWith("dbd/"))
    ...
    const safePath = relativePath
      ...
      .join("/");
    return `${BASE_GITHUB_URL}/${safePath}`;
  }
  ...
  // 6. Embed (O Cartão Visual)
  const embed = {
    // Título dentro do cartão (opcional, já que o post terá título)
  ...
  // 7. PAYLOAD (AQUI ESTÁ A MÁGICA DO FÓRUM)
  const payload = {
    username: "A Entidade",
    avatar_url:
    ...
    // Essa propriedade faz o webhook criar um Tópico novo!
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Erro Discord:", await response.text());
    } else {
