...
import { getFirestore, doc, onSnapshot, getDoc, setDoc } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
import { collection, query, orderBy } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
document.getElementById('page-main-title').innerText = `CHAVES - TORNEIO ${currentCamp}`;
...
function renderMenus() {
    ...
    menuTeams.innerHTML = ''; menuBrackets.innerHTML = '';
    let activeCamps = [];
...
window.assignTeamToSlot = async (teamId) => {
    try {
        await setDoc(bracketRef, { [activeSlotId]: { teamId: teamId, status: null } }, { merge: true });
        window.closeModal('select-team-modal');
    } catch(e) { alert("Erro ao salvar chave."); }
};

window.clearSlot = async () => {
    if(!activeSlotId) return;
    try { await setDoc(bracketRef, { [activeSlotId]: null }, { merge: true }); window.closeModal('select-team-modal');
    } catch(e) { alert("Erro ao limpar chave."); }
};

window.closeModal = (id) => document.getElementById(id).classList.remove('open');
window.closeModalOut = (e, id) => { if(e.target.id === id) window.closeModal(id); };
document.getElementById('user-profile-trigger').onclick = (e) => {
    e.stopPropagation(); document.getElementById('user-submenu').classList.toggle('hidden');
};
...
window.criarCampeonato = async (e) => {
    ...
    try {
        ...
        if (!snap.exists()) {
            ...
            let updates = {};
            if (window.nextCampId > currentTotalCamps) updates.totalCampeonatos = window.nextCampId;
            else if (deletedCamps.includes(window.nextCampId)) updates.deletedCamps = deletedCamps.filter(id => id !== window.nextCampId);
            if (Object.keys(updates).length > 0) await updateDoc(refDoc, updates);
        }
        window.location.href = `teams.html?camp=${window.nextCampId}`;
    } catch(err) { alert("Erro ao criar campeonato. Permissão Negada."); }
};